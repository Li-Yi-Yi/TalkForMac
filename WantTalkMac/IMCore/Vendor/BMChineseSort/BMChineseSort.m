...
// 给对象数组排序，返回的是对象元素数组
+ (NSMutableArray *)sortSingleObjectArray:(NSArray *)objectArray Key:(NSString *)key {
    //计算函数耗时
    CFAbsoluteTime startTime = CFAbsoluteTimeGetCurrent();
...
+(NSMutableArray*)ReturnSortObjectArrar:(NSArray*)stringArr key:(NSString *)key{
    ...
    for(int i=0;i<[stringArr count];i++){
        ...
        chineseString.string = [NSString stringWithString:[NSString stringWithFormat:@"%@", [temp valueForKeyPath:key]]];
